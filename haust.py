    ...
    while True:
        ...
        try:
            ...
            if "msg" in result and "Txhash" in result["msg"]:
                ...
                log_error(f"    ❌ Limit claim faucet for {address}. Waiting 60 second.")
                time.sleep(65)
            else:
...
def get_recipient_address(option: int, private_keys: List[str], current_index: int, addresses: List[str]) -> str:
    if option == 1:
        # Random address
        ...
        return addresses[current_index % len(addresses)]
    elif option == 3:
...
def transfer_eth():
    ...
    for idx, key in enumerate(private_keys):
        ...
        try:
            ...
            log_info(f"    💲 Balance : {eth_balance} {CURRENCY_SYMBOL}")
        except Exception as e:
        ...
        for i in range(num_transactions):
            ...
            recipient = get_recipient_address(recipient_option, private_keys, idx, addresses)
            # amount = random.uniform(0.01, 0.05)  # Amount in HAUST
            amount = 0.2
            ...
            try:
                ...
                log_info(f"    🎁 Transaction {i+1} Sending {amount} HAUST to {recipient}...")
